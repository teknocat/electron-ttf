...
実行:
- `bash internals/scripts/migration-electron-probe.sh 22.11.0:40.0.0`

結果:
- PASS
- ログ: `.artifacts/migration/node-22.11.0-electron-40.0.0.log`
...
## ホスト `npm install` 再検証 (2026-03-08)
...
補足:
- ログ中に `electron-builder install-app-deps` の `posix` 再ビルド失敗スタックが表示されるが、
  `postinstall` で許容済み (`|| echo ...`) のためインストール全体は成功。
- `@parcel/watcher-*` の `SKIPPING OPTIONAL DEPENDENCY` は Linux x64 では想定内。
- 現時点で「npm install がエラーで止まる」状態は再現せず、次工程 (`npm run dev`) へ進行可能。

## 最新 Electron 反映とホスト起動確認 (2026-03-08)

対応:
- `package.json` の `devDependencies.electron` を `^2.0.11` から `^40.8.0` へ更新。
- Electron 40 系では `electron -r babel-register ./app/main.dev.js` が ESM 扱いで失敗するため、
  `start-main-dev` を `electron ./app` 起動へ変更。

確認:
- `node_modules/electron/package.json` で `40.8.0` を確認。
- `npm run dev` を再実行し、`EXIT_CODE=0` を確認。

補足:
- ログ上の `legacy-js-api` (sass) / `dbus` / `MESA` 警告は出るが、今回の起動成功判定は維持。

## Node 24 + npm 11 ホスト起動整備 (2026-03-08)

事象:
- Node `24.14.0` + npm `11.9.0` で `npm run dev` 実行時、
  `Invalid property "devEngines.node"` で起動前に失敗。
- 上記解消後、renderer 側で webpack4/OpenSSL3 由来の
  `ERR_OSSL_EVP_UNSUPPORTED` が再発。

対応:
- `package.json` の `devEngines` を `engines` へ変更。
- `postinstall` 末尾の `check-dev-engines.js` 呼び出しを削除。
- `start-renderer-dev` に `NODE_OPTIONS=--openssl-legacy-provider` を付与。
- `start-main-dev` は `NODE_OPTIONS=` でクリアして `electron ./app` 起動を維持。

確認:
- Node `24.14.0` / npm `11.9.0` / Electron `40.8.0` で `npm run dev` 実行。
- `EXIT_CODE=0` を確認（致命エラーなし）。
